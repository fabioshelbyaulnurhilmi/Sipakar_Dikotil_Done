                        ...
                        <tr>
                            ...
                            <td class="d-none d-sm-table-cell fs-sm text-center">
                                @if ($key->status == 'Tidak Valid')
                                    Penyakit yang muncul dari gejala yang dipilih ada 2 kemungkinan yaitu
                                    @foreach ($key->DiagnosaToDetail->unique('RelasidetailPenyakit.DetailPenyakitToPenyakit.namaPenyakit') as $item)
                                        {{ $item->RelasidetailPenyakit->DetailPenyakitToPenyakit->namaPenyakit . ' (' . $item->RelasidetailPenyakit->DetailPenyakitToPenyakit->idPenyakit . ')' }}
                                        @if (!$loop->last)
                                            dan
                                        @endif
                                    @endforeach
                            ...
                            <td class="text-center">
                                @if ($key->status == 'Tidak Valid')
                                    Silahkan Diagnosa Ulang untuk menentukan penyakit tanaman anda dikarenakan
                                    @foreach ($key->DiagnosaToDetail as $item)
                                        {{ $item->RelasidetailPenyakit->DetailPenyakitToGejala->namaGejala }} merupakan
                                        tanda dari penyakit
                                        {{ $item->RelasidetailPenyakit->DetailPenyakitToPenyakit->namaPenyakit . ' (' . $item->RelasidetailPenyakit->DetailPenyakitToPenyakit->idPenyakit . ')' }}
                                        @if (!$loop->last)
                                            dan
                                ...
                                @else
                                    <div class="btn-group">
                                        <a href="{{ route('detail-diagnosa', $key->DiagnosaToDetail[0]->key) }}"
                                            type="button" style="background-color: rgb(216, 84, 84)"
