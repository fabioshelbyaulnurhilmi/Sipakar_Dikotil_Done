...
class LaporanController extends Controller
{
    ...
    public function cetak_coba($id)
    {
        // $id = 'detaildiagnosa-1683099187dxu2upqdb164520e3343fa1';
        $diagnosa = detailDiagnosa::where('key', $id)->get();
        $diagnosas = Diagnosa::with('DiagnosaToDetail')->whereHas('DiagnosaToDetail', function ($q) use ($id) {
            $q->where('key', $id);
        })->get();
        return view('admin.dataLaporan.cetak', compact(['diagnosa', 'diagnosas']));
    }
    ...
    public function komen(Request $request)
    {
        ...
        if (Auth::user()->UserToDiagnosa->isEmpty()) {

            $rating->penyakit = null;
        } else {
            // dd(Auth::user()->UserToDiagnosa[0]->DiagnosaToDetail[0]->RelasidetailPenyakit->DetailPenyakitToPenyakit->namaPenyakit);
            $rating->penyakit = Auth::user()->UserToDiagnosa[0]->DiagnosaToDetail[0]->RelasidetailPenyakit->DetailPenyakitToPenyakit->namaPenyakit;
        }
    ...
    public function delete($id)
    {
        ...
        return redirect()->route('komentar')
