...
class dataPakarController extends Controller
{
    ...
    {
        $editGejala = Gejala::where('idGejala', $id)->first();
    ...
    {
        $editPenyakit = Penyakit::where('idPenyakit', $id)->first();
    ...
    public function simpanGejala(Request $request)
    {
        ...
        if ($request->gambarGejala2) {
            $image_name2 = uniqid() . '.' . $request->gambarGejala2->getClientOriginalExtension();
            $image_path2 = 'imagesGejala/' . $image_name2;
            Image::make($request->gambarGejala2->getRealPath())->fit(500, 500)->save(storage_path('app/public/' . $image_path2));
        } else {
            $image_path2 = null;
        }

        if ($request->gambarGejala3) {
            $image_name3 = uniqid() . '.' . $request->gambarGejala3->getClientOriginalExtension();
            $image_path3 = 'imagesGejala/' . $image_name3;
            Image::make($request->gambarGejala3->getRealPath())->fit(500, 500)->save(storage_path('app/public/' . $image_path3));
        } else {
            $image_path3 = null;
        }
        Gejala::create([
            ...
            'gambarGejala2' => $image_path2,
            'gambarGejala3' => $image_path3,
            'key' => ' ',
        ]);
    ...
    public function updateGejala(Request $request)
    {
        ...
        if ($request->gambarGejala2) {
            if (Storage::exists('public/' . $dGejala->gambarGejala2)) {
                Storage::delete('public/' . $dGejala->gambarGejala2);
                // $this->alert('success', 'gambar Berhasil Diupdate');
            };
            ...
            $image_path2 = $dGejala->gambarGejala2;
        }
        if ($request->gambarGejala3) {
            if (Storage::exists('public/' . $dGejala->gambarGejala3)) {
                Storage::delete('public/' . $dGejala->gambarGejala3);
                // $this->alert('success', 'gambar Berhasil Diupdate');
            };
            ...
            $image_path3 = $dGejala->gambarGejala3;
        }
    ...
    public function deleteGejala(Request $request, $id)
    {
        ...
        if (!$gejala) {
            Alert::success('GAGAL', 'Menghapus Data Gejala');
            return redirect()->route('data-gejala')
        ...
        Storage::delete('public/' . $gejala->gambarGejala2);
        Storage::delete('public/' . $gejala->gambarGejala3);
        ...
        Alert::success('Berhasil', 'Menghapus Gejala');
        return redirect()->route('data-gejala')
    ...
    public function deletePenyakit(Request $request, $id)
    {
        ...
        if (!$penyakit) {
            Alert::success('GAGAL', 'Menghapus Data Penyakit');
            return redirect()->route('data-penyakit')
                ->with('error', 'User not found');
        }
        $penyakit->delete();
    ...
    public function deleteObat(Request $request, $idObat)
    {
        ...
        if (!$obat) {
            Alert::success('GAGAL', 'Menghapus Data Obat');
            return redirect()->route('data-obat')
                ->with('error', 'User not found');
        }
        $obat->delete();
